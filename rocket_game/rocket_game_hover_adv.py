import pygame
import physics
from rocket_hover_adv import Rocket
...
class Game:
    def __init__(self, render=True, agent_play=True, agent_train=True, agent_file='rocket_game_hover_adv', agent_load_file=None, save_episodes=100,
        ...
        self.x_target = self.rng.normal(0.0, 20.0)
        self.y_target = -30.0 + self.rng.normal(0.0, 15.0)
    ...
    def _update(self, dt, state, action=None):
        ...
        done = engine_on_ground or nose_on_ground or out_of_bounds_left or out_of_bounds_right or out_of_bounds_top
    ...
    def _render(self):
        ...
        # Target
        target_circ = pygame.draw.circle(self.display_surf, (20, 20, 250), (self.width/2 + self.x_target * ppm, self.height - (physics.ground_height - self.y_target) * ppm), 3)

        new_dirty_rects = [rocket_circ1, rocket_circ2, rocket_line, burn_line, nozzle_line, target_circ]
...
if __name__ == "__main__":
    ...
    game = Game(render=True, agent_play=True, agent_train=True, agent_file='rocket_game_hover_adv', agent_load_file='rocket_game_hover_e1000',
                save_episodes=100, step_limit=2000, device=device)
    game.play()